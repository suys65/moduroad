...
def find_modu_path(start, end, network, speed_kmph=3.7):
    """
    start: 시작점 좌표 (위도, 경도)
    end: 도착점 좌표 (위도, 경도)
    network: 네트워크 그래프
    ...
    # A* 알고리즘으로 최단 경로 찾기
    route = nx.astar_path(G, start_node, end_node, weight='w_weight')  # 여기서 'weight'를 'length'로 수정해야 합니다.
...
# 네트워크 그래프 파일 경로
network_file_path1 = 'C:/my_code/moduroad/moduroad_app/cashe_data/api_graph.pickle'
...
if st.sidebar.button('Find Shortest Path'):
    ...
    with open(network_file_path1, 'rb') as file:
        network_g1 = pickle.load(file)
    combined_line_string1, total_length1, total_time1 = find_shortest_path(start, end, network_g1, speed_kmph=speed_kmph)
    
    # 네트워크 그래프 로딩 및 최단 경로 계산 (두 번째 네트워크)
    with open(network_file_path2, 'rb') as file:
        network_g2 = pickle.load(file)
    combined_line_string2, total_length2, total_time2 = find_modu_path(start, end, network_g2, speed_kmph=speed_kmph)
